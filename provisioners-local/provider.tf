terraform {
  ...
  backend "s3" {
    ...
    region = "us-east-1"
    encrypt = true #enabling encryption
    use_lockfile = true #enabling s3 native locking
    #dynamodb_table = "84s-remote-state" #Any name we can give.There is no rule that it should be match with bucket name.
  }
}

/* If we use encrypt and use_lockfile then no need to use dynamoDB, because use_lockfile is native locing methodolgy for S3 */
